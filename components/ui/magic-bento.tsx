...
export default function MagicBento({
  ...
  const cardClassBase = useMemo(() => {
    return `card ${textAutoHide ? "card--text-autohide" : ""} ${enableBorderGlow ? "card--border-glow" : ""}`;
  ...
  return (
    <div className={className}>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {cards.map((card) => {
          const baseClassName = `${cardClassBase} ${card.className ?? ""}`.trim();
          const style: CSSProperties = {
            backgroundColor: card.accentColor ?? "#060010",
          ...
          return (
            <div key={card.id} className={baseClassName} style={style}>
              <CardContent card={card} />
            </div>
          );
        })}
  ...
  return (
    <>
      ...
      <div className="card__content">
        {card.title ? <h2 className="card__title">{card.title}</h2> : null}
