...
function extendMaterial<T extends typeof THREE.MeshStandardMaterial>(BaseMaterial: T, cfg: {
  ...
  Object.entries(cfg.vertex ?? {}).forEach(([includeKey, code]) => {
    vert = vert.replace(includeKey, `${includeKey}\n${code}`);
  });

  Object.entries(cfg.fragment ?? {}).forEach(([includeKey, code]) => {
    frag = frag.replace(includeKey, `${includeKey}\n${code}`);
  });
...
type MergedPlanesProps = PlaneNoiseProps;

type MeshRef = THREE.Mesh<THREE.BufferGeometry, THREE.ShaderMaterial>;

const MergedPlanes = forwardRef(function MergedPlanes(
  { material, width, count, height }: MergedPlanesProps,
  ref: ForwardedRef<MeshRef>
) {
  const mesh = useRef<MeshRef | null>(null);
...
const PlaneNoise = forwardRef(function PlaneNoise(
  props: PlaneNoiseProps,
  ref: ForwardedRef<MeshRef>
) {
  return <MergedPlanes ref={ref} {...props} />;
});
